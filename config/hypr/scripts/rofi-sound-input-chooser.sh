...
then
    ...
    device=$(pactl list sources | grep -v "monitor" | grep -C2 -F "Description: $desc"|grep Name|cut -d: -f2|xargs)
    ...
    for x in $(pactl list sources | grep -v "Monitor" | grep -ie "description:"|cut -d: -f2|sort)
    do
